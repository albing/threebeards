...
public class RockOfDoomTriggerLevel1Part1 : MonoBehaviour {
	...
	void Start(){
		cadFlag.transform.position = new Vector2 (44.81f, -27.69f);
	...
	void OnTriggerEnter2D(Collider2D other) {
		if (other.name == "Cowboy")
		{
			if (Shoot.score < maxScore)
			{
				...
				if(!hasBeenTriggered)
				{
					redLight.transform.position = new Vector3 (44.81f, -27.69f, -1.7f);
					red = Instantiate (redLight);
					usaFlag.transform.position = new Vector2 (44.81f, -27.69f);
	...
	{
		blueLight.transform.position = new Vector3 (44.81f, -27.69f, -1.7f);
		DestroyObject (red);
